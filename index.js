...
if (forgotForm) {
  forgotForm.addEventListener('submit', async function (e) {
    ...
    if (!email) {
      setForgotMessage('Please enter your email address.', true);
      return;
    }
...
<div id="forgot-backdrop" class="auth-backdrop">
  <div class="auth-modal forgot-modal">
    <button type="button" class="auth-close" id="forgot-close">×</button>
    ...
    <form id="forgot-form">
      <input
        ...
        autocomplete="email"
      />
      <button type="submit" id="forgot-submit-btn">Send reset link</button>
    </form>

    <div id="forgot-error" class="auth-error" style="display:none;margin-top:12px;"></div>
  </div>
</div>
...
app.post('/forgot-password', async (req, res) => {
  try {
    ...
    if (error) {
      console.error('forgot-password profile lookup error:', error);
      return res.json({
        ok: true,
        exists: false,
        message: 'If an account exists with that email, a reset link has been sent.',
